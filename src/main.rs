...
fn main() {
    ...
    let voxel_map = if ext == "litematic" {
        ...
        match result {
            Ok(schem) => match parser::VoxelMap::from_litematic(&schem) {
                Ok(map) => Some(map),
                Err(e) => {
                    eprintln!("Failed to parse litematic {}: {}", args.input.display(), e);
                    None
                }
            },
            Err(e) => {
        ...
        match nbt_unpack::load_schematic(&args.input) {
            Ok(data) => {
                ...
                match parser::parse_nbt(&data) {
                    Ok(map) => Some(map),
                    Err(e) => {
                        eprintln!("Failed to parse NBT schematic {}: {}", args.input.display(), e);
                        None
                    }
                }
            }
